...
# draw plot
title <- paste0("# subjects=",ns,", mean fold-change=",fc,",\n# cells per subject=",ncps)
p1 <- ggplot(df1, aes(x=nebula, y=glmgampoi), element_text(size=15)) +
...
p3 <- grid.arrange(p1, nrow=1, top=title)

# save image
saveRDS(p3, out_path)
#ggsave(out_path, plot = p3, width=4, height=4)
